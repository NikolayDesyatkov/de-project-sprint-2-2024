...
dwh_delta_insert_result AS ( -- делаем расчёт витрины по новым данным. 
    SELECT  
            ...
            T6.product_sum AS customer_money,
            T6.product_sum*0.1 AS platform_money,
            ...
            T5.top_craftsman_customer  as top_craftsman, 
                        ...
                        *,
                        ...
                        FROM ( 
                            SELECT -- в этой выборке делаем расчёт по большинству столбцов, так как все они требуют одной и той же группировки, кроме столбца с самой популярной категорией товаров у мастера. Для этого столбца сделаем отдельную выборку с другой группировкой и выполним JOIN
                              ...
                              --  SUM(T1.product_price) AS customer_money, ---убираем т.к. не нужна группировка суммы по месяцам
                               --SUM(T1.product_price) * 0.1 AS platform_money, ---убираем т.к. не нужна группировка суммы по месяцам
                                ...
                                INNER JOIN (
                                    ...
                                    --Для данных этой выборки можно применить оконную функцию, которая и покажет самую 
                                    --популярную категорию товаров у заказчика
                                            dd.customer_id AS customer_id_for_product_type, 
                                            dd.product_type, 
                                            dd.report_period as r_period,
                                            COUNT(dd.product_id) AS count_product
                                            FROM dwh_delta AS dd
                                                GROUP BY dd.customer_id, dd.product_type, dd.report_period
                                                    ORDER BY count_product DESC) AS T3 ON T2.customer_id = T3.customer_id_for_product_type and T3.r_period=T2.report_period                   
                                                    ) AS T4 
                                                    
                        --производим подсчет заказов у каждого клиента с группировкой и счетчиком по мастеру, без учета периода
                        inner join (
                        select *,
                      	FIRST_VALUE(top_crafsman) OVER (PARTITION BY ST1.cstomer_id ORDER BY count_orders DESC) as top_craftsman_customer 
                        from ( select
						dd.customer_id as cstomer_id,
						dd.craftsman_id as top_crafsman, 
						count(dd.order_id) as count_orders
						from dwh_delta AS dd
						group by dd.customer_id,dd.craftsman_id) as ST1
                ) as T5  on T4.customer_id=T5.cstomer_id
  
                --считаем сумму заказов не зависимо от периода
                inner join 
                (
                select 
                dd.customer_id,
                SUM(dd.product_price) as product_sum
                from dwh_delta dd
                group by dd.customer_id
                ) as T6 on T6.customer_id=T4.customer_id
                                                    
                                                    
                WHERE T4.rank_count_product = 1  ORDER BY report_period -- условие помогает оставить в выборке первую по популярности категорию товаров
),
dwh_delta_update_result AS ( -- делаем расчёт витрины по новым данным. 
    SELECT  
            T4.customer_id AS customer_id,
            T4.customer_name AS customer_name,
            T4.customer_address AS customer_address,
            ...
            FROM (
                SELECT     -- в этой выборке объединяем две внутренние выборки по расчёту столбцов витрины и применяем оконную функцию для определения самой популярной категории товаров
                        *,
                        ...
                        inner join (
                        select *,
                       -- RANK() OVER(PARTITION BY ST1.cstomer_id ORDER BY count_orders DESC) AS rank_count_orders 
                    	FIRST_VALUE(top_crafsman) OVER (PARTITION BY ST1.cstomer_id ORDER BY count_orders DESC) as top_craftsman_customer
                        from ( select
						dd.customer_id as cstomer_id,
						dd.craftsman_id as top_crafsman, 
